...
const RecentScanCard = ({ scan, onResolve }) => {
  ...
  const getRiskBgColor = (score, status) => {
    if (status === 'resolved') return 'bg-success/20 border-success/30';
  ...
  const getRiskLabel = (score, status) => {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3 md:mb-4">
        ...
        <div className={`px-3 py-1 rounded-full border ${getRiskBgColor(scan?.riskScore, scan?.status)} flex-shrink-0 ml-2`}>
          <span className={`text-xs md:text-sm font-medium ${scan?.status === 'resolved' ? 'text-success' : getRiskColor(scan?.riskScore)}`}>
            {getRiskLabel(scan?.riskScore, scan?.status)}
          </span>
        </div>
      </div>
